...
{
    public class DefaultProbabilityStorage<TKey, TValue> : IStorage<TKey, TValue>
    {
        public IRandom Random;
        public DefaultProbabilityStorage() : this(new DefaultRandom()) { }
        public DefaultProbabilityStorage(IRandom rng)
        {
            Random = rng;
        }

        public ConcurrentDictionary<TKey, Dictionary<TValue, DataValues>> Store = new ConcurrentDictionary<TKey, Dictionary<TValue, DataValues>>();
        ...
        {
            bool exists = Store.TryGetValue(key, out Dictionary<TValue, DataValues> node);
            if (exists)
            {
                value = GetValue(node);
            }
        ...
        public void Set(TKey key, TValue value)
        {
            if (Store.ContainsKey(key))//Table already exists
            {
                ...
                {
                    Store[key].Add(value, new DataValues());
                }
            }
            else//Table doesnt exist - Create table and add key
            {
                Store.TryAdd(key, new Dictionary<TValue, DataValues>()
                {
